"use client"

import { useState, useEffect, useRef } from "react"
import axios from "axios"
import "./Reclamation.css"
import Afficherreclamation from "./Afficherreclamation"
import { BASE_URL } from "../config"

const Reclamation = ({ userRole, userId = 101 }) => {
  const [claims, setClaims] = useState([])
  const [filteredClaims, setFilteredClaims] = useState([])
  const [showForm, setShowForm] = useState(false)
  const [selectedClaim, setSelectedClaim] = useState(null)
  const [showClaimDetails, setShowClaimDetails] = useState(false)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)
  const [teachers, setTeachers] = useState([])

  // Add new state for teachers list
  const [teachersList, setTeachersList] = useState([])

  // Resolve role: prefer explicit prop, otherwise try stored user role
  const storedUserForInit = (() => {
    try {
      return JSON.parse(localStorage.getItem("user")) || null
    } catch (e) {
      return null
    }
  })()
  const [resolvedRole, setResolvedRole] = useState(userRole || (storedUserForInit && storedUserForInit.role) || null)

  useEffect(() => {
    if (userRole && userRole !== resolvedRole) setResolvedRole(userRole)
  }, [userRole])

  // Form state
  const [formData, setFormData] = useState({
    teacherId: "",
    title: "",
    description: "",
  })

  const formRef = useRef(null)

  // Small helpers to read token and user from localStorage (support legacy userToken)
  const getToken = () => localStorage.getItem("token") || localStorage.getItem("userToken")
  const getStoredUser = () => {
    try {
      return JSON.parse(localStorage.getItem("user")) || null
    } catch (e) {
      return null
    }
  }

  // Reusable DOM notification (top-right). type: 'success' | 'error'
  const showNotification = (message, type = 'success') => {
    const bg = type === 'success' ? '#3DDC97' : '#EF4444'
    const notification = document.createElement('div')
    notification.className = 'fixed top-4 right-4 text-white px-4 py-2 rounded-lg shadow-lg z-50 flex items-center'
    notification.style.background = bg
    notification.innerHTML = `
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
      </svg>
      <span>${message}</span>
    `
    document.body.appendChild(notification)
    setTimeout(() => {
      notification.classList.add('opacity-0', 'transition-opacity', 'duration-500')
      setTimeout(() => document.body.removeChild(notification), 500)
    }, 3000)
  }

  // Fetch teachers for students
  useEffect(() => {
    const fetchTeachers = async () => {
      try {
        const token = getToken()
        console.debug("Reclamation: fetchTeachers token:", token)
        const response = await axios.get(`${BASE_URL}/api/users?role=teacher`, {
          headers: {
            Authorization: token ? `Bearer ${token}` : ""
          }
        })
        const teachersList = response.data.users.map(teacher => ({
          id: teacher._id,
          name: `${teacher.firstName} ${teacher.lastName}`
        }))
        setTeachers(teachersList)
        setTeachersList(response.data.users)
      } catch (err) {
        console.error("Reclamation: failed to load teachers", err)
        setError("Failed to load teachers")
      }
    }

    if (resolvedRole === "student") {
      fetchTeachers()
    }
  }, [resolvedRole])

  useEffect(() => {
    // removed: consolidated into the more capable fetch below
  }, [resolvedRole])

  // Handle form submission
  const handleSubmitClaim = async () => {
    if (!formData.teacherId || !formData.title || !formData.description) {
      showNotification("Veuillez remplir tous les champs", 'error')
      return
    }

    try {
      const token = getToken()
      const response = await axios.post(
        `${BASE_URL}/api/claims`,
        {
          teacher: formData.teacherId,
          title: formData.title,
          details: formData.description
        },
        {
          headers: {
            Authorization: token ? `Bearer ${token}` : "",
            'Content-Type': 'application/json'
          }
        }
      )

      if (response.data && response.data.claim) {
        // Reset form and close
        setFormData({ teacherId: "", title: "", description: "" })
        setShowForm(false)

        // Show green top-right notification (same style used elsewhere)
        const notification = document.createElement("div")
        notification.className = "fixed top-4 right-4 bg-[#3DDC97] text-white px-4 py-2 rounded-lg shadow-lg z-50 flex items-center"
        notification.innerHTML = `
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
          </svg>
          <span>Réclamation envoyée avec succès !</span>
        `
        document.body.appendChild(notification)
        setTimeout(() => {
          notification.classList.add("opacity-0", "transition-opacity", "duration-500")
          setTimeout(() => document.body.removeChild(notification), 500)
        }, 3000)

        // Re-fetch claims to avoid undefined fields and sync with server
        await fetchClaims()
      }
    } catch (err) {
      console.error("Error submitting claim:", err)
      showNotification(err.response?.data?.message || "Erreur lors de l'envoi de la réclamation", 'error')
    }
  }

  // Handle claim status update
  const handleUpdateClaimStatus = async (claimId, newStatus, response = "") => {
    try {
      const token = getToken()
      if (!token) {
        showNotification("Authentication required", 'error')
        return
      }

      await axios.put(
        `${BASE_URL}/api/claims/${claimId}/respond`,
        {
          status: newStatus,
          response: response
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
        }
      )

      const updatedClaims = filteredClaims.map((claim) => {
        if (claim.id === claimId) {
          return { 
            ...claim, 
            status: newStatus, 
            response,
            isResponded: true,
            updatedAt: new Date().toISOString()
          }
        }
        return claim
      })

      setFilteredClaims(updatedClaims)
      setSelectedClaim(null)
      setShowClaimDetails(false)
      showNotification(newStatus === "dealt" ? "Réclamation traitée avec succès!" : "Réclamation rejetée avec succès!", 'success')
    } catch (err) {
      showNotification("Erreur lors de la mise à jour de la réclamation", 'error')
    }
  }

  // Handle claim selection
  const handleSelectClaim = (claim) => {
    setSelectedClaim(claim)
    setShowClaimDetails(true)
  }

  // Handle click outside form
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (formRef.current && !formRef.current.contains(event.target)) {
        setShowForm(false)
      }
    }

    if (showForm) {
      document.addEventListener("mousedown", handleClickOutside)
    }

    return () => {
      document.removeEventListener("mousedown", handleClickOutside)
    }
  }, [showForm])

  // Fetch student details helper
  const fetchClaimDetails = async (studentId) => {
    try {
      const token = getToken();
      const response = await axios.get(`${BASE_URL}/api/users/${studentId}`, {
        headers: { Authorization: token ? `Bearer ${token}` : "" }
      });
      return response.data.user;
    } catch (err) {
      console.error("Error fetching student details:", err);
      return null;
    }
  };

  // Fetch claims (lifted so other handlers can call it)
  const fetchClaims = async () => {
    if (!resolvedRole) return
    try {
      const token = getToken();
      console.debug("Reclamation: fetchClaims token:", token, "resolvedRole:", resolvedRole)
      if (!token) {
        setError("Authentication required")
        setLoading(false)
        return
      }

      const endpoint = resolvedRole === "teacher"
        ? "/api/claims/received"
        : resolvedRole === "student"
          ? "/api/claims/my-claims"
          : "/api/claims/all"

      const response = await axios.get(`${BASE_URL}${endpoint}`, {
        headers: { Authorization: token ? `Bearer ${token}` : "" }
      });

      const raw = response.data
      const list = Array.isArray(raw) ? raw : raw.claims || raw

      const mappedClaims = await Promise.all(list.map(async (claim) => {
        let studentDetails = null;
        if (resolvedRole === "teacher" && claim.student) {
          studentDetails = await fetchClaimDetails(claim.student);
        }

        return {
          id: claim._id,
          title: claim.title,
          description: claim.details,
          teacherId: claim.teacher?._id,
          teacherName: claim.teacher ? `${claim.teacher.firstName} ${claim.teacher.lastName}` : "",
          studentName: resolvedRole === "teacher"
            ? (studentDetails ? `${studentDetails.firstName} ${studentDetails.lastName}` : "Étudiant inconnu")
            : (() => {
                const stored = getStoredUser();
                return stored ? `${stored.firstName} ${stored.lastName}` : "";
              })(),
          studentId: claim.student,
          date: new Date(claim.createdAt).toLocaleDateString('fr-FR'),
          status: claim.status,
          response: claim.response || "",
          isResponded: claim.isResponded,
          updatedAt: claim.updatedAt ? new Date(claim.updatedAt).toLocaleDateString('fr-FR') : null
        };
      }));

      setFilteredClaims(mappedClaims);
      setLoading(false);
    } catch (err) {
      console.error("Error fetching claims:", err);
      setError("Failed to load claims");
      setLoading(false);
    }
  }

  useEffect(() => {
    fetchClaims();
  }, [resolvedRole])


  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#3ddc97]"></div>
      </div>
    )
  }

  if (error) {
    return (
      <div className="text-center text-red-500 p-4">
        {error}
      </div>
    )
  }

  return (
    <div className="Appreclamation">
      <div className="containerreclamation">
        <div className="mainreclamation">
          {/* Claims display */}
          <div className="claims-container">
            <Afficherreclamation
              claims={filteredClaims}
              onSelectClaim={handleSelectClaim}
              selectedClaim={selectedClaim}
              showDetails={showClaimDetails}
              onCloseDetails={() => setShowClaimDetails(false)}
              onUpdateStatus={handleUpdateClaimStatus}
              userRole={resolvedRole}
              teachers={teachers}
            />
          </div>

          {/* Add claim button (only for students) */}
          {resolvedRole === "student" && (
            <button className="buttonpourajouterreclamation" onClick={() => setShowForm(true)}>
              +
            </button>
          )}

          {/* Claim form */}
          {showForm && resolvedRole === "student" && (
            <div className="lesreclamations" ref={formRef}>
              <div className="distination">
                <span className="distination1">À</span>
                <select
                  className="distination2"
                  value={formData.teacherId}
                  onChange={(e) => setFormData({ ...formData, teacherId: e.target.value })}
                >
                  <option value="">Sélectionner un enseignant...</option>
                  {teachers.map((teacher) => (
                    <option key={teacher.id} value={teacher.id}>
                      {teacher.name}
                    </option>
                  ))}
                </select>
              </div>

              <div className="objet">
                <span className="objet2">Objet</span>
                <input
                  type="text"
                  className="objet3"
                  value={formData.title}
                  onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                  placeholder="Titre de votre réclamation..."
                />
              </div>

              <textarea
                className="reclamation"
                placeholder="Décrivez votre réclamation en détail..."
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
              ></textarea>

              <button className="buttondemessage" onClick={handleSubmitClaim}>
                <span>ENVOYER</span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <line x1="22" y1="2" x2="11" y2="13"></line>
                  <polygon points="22 2 15 22 11 13 2 9 22 2"></polygon>
                </svg>
              </button>
            </div>
          )}
        </div>
      </div>
    </div>
  )
}

export default Reclamation
