  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Location selector with conditional fields */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          {(() => {
            ...
            // Derive a locationType from existing newItem.location if present
            const deriveType = () => {
              if (!newItem?.location) return newItem?.locationType || ''
              if (newItem.location.startsWith('Amphi')) return 'Amphi'
              if (places.includes(newItem.location)) return newItem.location
              return 'Autre'
            }

            const locationType = deriveType()
              ...
              <div>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (v === 'Amphi') {
                      // default to Amphi 1 if not set
                      setNewItem({ ...newItem, locationType: 'Amphi', location: 'Amphi 1', amphiNumber: 1 })
                    } else if (v === 'Autre') {
                      setNewItem({ ...newItem, locationType: 'Autre', location: '' })
                    } else {
                      setNewItem({ ...newItem, locationType: v, location: v })
                    }
                  }}
                >
                ...
                {/* Amphi number selector */}
                {locationType === 'Amphi' && (
                  <select
                    className="w-full p-2 mb-2 rounded border border-gray-300 dark:border-gray-600 bg-[#f4f7f9] dark:bg-[#2a2a3d] text-sm focus:outline-none focus:border-[#A0E6D7] focus:ring-2 focus:ring-[#A0E6D7] transition-all duration-300"
                    value={newItem?.amphiNumber || 1}
                    onChange={(e) => {
                      const num = Number(e.target.value)
                      setNewItem({ ...newItem, amphiNumber: num, location: `Amphi ${num}`, locationType: 'Amphi' })
                    }}
                  >
                    {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
                      <option key={n} value={n}>{`Amphi ${n}`}</option>
                    ))}
                  </select>
                )}
                ...
                {locationType === 'Autre' && (
                  <input
                    type="text"
                    placeholder="Où avez-vous perdu/trouvé cet objet?"
                    className="w-full p-2 mb-2 rounded border border-gray-300 dark:border-gray-600 bg-[#f4f7f9] dark:bg-[#2a2a3d] text-sm placeholder-gray-500 dark:placeholder-gray-400 transition-all duration-300 focus:ring-2 focus:ring-[#3ddc97] focus:border-[#3ddc97]"
                    value={newItem.location || ''}
                    onChange={(e) => setNewItem({ ...newItem, location: e.target.value, locationType: 'Autre' })}
                  />
                )}
              </div>
            )
