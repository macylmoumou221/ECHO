...
const Notification = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const rawNotifications = Array.isArray(response.data) ? response.data : response.data.notifications || []
      const formattedNotifications = rawNotifications.map((notification) => {
        ...
        return {
          ...
          onModel: notification.onModel || notification.on_model || null,
          sender: {
            id: senderId,
  ...
  const handleNotificationClick = async (item) => {
    try {
      ...
      // Prefer using notification.type === 'message' to go to messages using sender id
      if (item.type === 'message') {
        const senderId = item.sender?.id || item.raw?.sender?._id || null
        if (senderId) {
          console.debug('Navigating to messagerie for sender:', senderId)
          navigate(`/messagerie/${senderId}`)
          return
        }
      }

      // If notification references a relatedId and it's a post/publication, go to publication
      const relatedId = item.relatedId || item.raw?.relatedId || item.raw?.related_id || null
      const onModel = item.onModel || item.raw?.onModel || item.raw?.on_model || null
      if (relatedId && onModel) {
        // If backend says it's a Message, try to open the chat with sender
        if (onModel.toLowerCase() === 'message') {
          const senderId = item.sender?.id || item.raw?.sender?._id || null
          if (senderId) {
            navigate(`/messagerie/${senderId}`)
            return
        ...
        // For posts/publications
        if (onModel.toLowerCase() === 'post' || onModel.toLowerCase() === 'publication' || onModel.toLowerCase() === 'publication') {
          navigate(`/post/${relatedId}`)
          return
        }

        // Fallback: if onModel isn't clear, but relatedId exists, try publication route
        navigate(`/post/${relatedId}`)
        return
      }

      // Nothing to navigate to — log for debugging
      console.debug('No navigation target for notification item:', item)
    } catch (err) {
                  ...
                  {filterNotificationsByTab().length > 0 ? (
                    filterNotificationsByTab().map((item) => (
                      <div
                          ...
                          onClick={() => item.type !== "announcement" && handleNotificationClick(item)}
                        >
