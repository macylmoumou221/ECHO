...
// Return a friendly preview for latest messages (show photo captions when appropriate)
const formatPreviewText = (latestMessage, partnerName) => {
  if (!latestMessage) return ""
  // If there's media and no text, show a friendly caption
  if (latestMessage.media && (!latestMessage.text || latestMessage.text.trim() === "")) {
    // Determine whether message was sent by current user
    if (latestMessage.isFromUser || latestMessage.isFromCurrentUser || latestMessage.sent) {
      return "Vous avez envoyé une photo"
    }
    // Fallback to partner name if available
    const sender = partnerName || "Utilisateur"
    return `${sender} a envoyé une photo`
  }

  // Otherwise show the text (or empty string)
...
const Messagerie = () => {
  // Detect whether the app's global sidebar is collapsed and track its width
  const [isExternalSidebarClosed, setIsExternalSidebarClosed] = useState(() => {
  ...
  useEffect(() => {
    ...
    // Check for an <aside> sidebar element and observe its width using ResizeObserver.
    // Sidebar component uses Tailwind classes w-[230px] or w-[80px], but width is most reliable.
    ...
    if (aside && typeof ResizeObserver !== 'undefined') {
      const onResize = () => {
        ...
        // Update measured width and collapsed boolean
        setSidebarWidth(w)
        setIsExternalSidebarClosed(w <= 100)
      }
      ...
      // Fallback: observe body attribute/class changes and set approximate width
      const check = () => {
        const closed = checkBody()
        setIsExternalSidebarClosed(closed)
        setSidebarWidth(closed ? 80 : 230)
      }
      const mo = new MutationObserver(check)
      mo.observe(body, { attributes: true, attributeFilter: ['class', 'data-sidebar-collapsed'] })
      // run initial check
      check()
  ...
  // When isExternalSidebarClosed changes, update the CSS variable on the root element
  useEffect(() => {
    if (typeof document === 'undefined') return
    const root = document.querySelector('.Appmessagerie')
    if (!root) return
    // Set the offset to the measured sidebar width so the messenger sits next to it
    root.style.setProperty('--sidebar-offset', `${sidebarWidth}px`)
  }, [isExternalSidebarClosed, sidebarWidth])
  ...
  const refreshCurrentChat = async (contactId) => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/api/messages/${contactId}`, {
  ...
  const handleContactSelect = async (contact) => {
    try {
      ...
      const token = localStorage.getItem("token")
      const response = await axios.get(`${BASE_URL}/api/messages/${contact.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
    ...
    <div key={`${msg.id}-${index}`} className={`message-wrapper ${msg.sent ? "sent" : "received"}`}>
      <div className={`message ${msg.sent ? "message-sent" : "message-received"}`}>
            <div className="message-content">
              {/* If message has text, show it. If not but has media, show a friendly caption */}
              {msg.text && <div className="message-text">{msg.text}</div>}
              {!msg.text && msg.media && (
                <div className="message-text message-media-caption">
                  {msg.sent ? "Vous avez envoyé une photo" : `${selectedContact?.name || "Utilisateur"} a envoyé une photo`}
                </div>
              )}
              {msg.media && (
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Let Axios set the correct multipart/form-data boundary header.
      const response = await axios.post(`${BASE_URL}/api/messages/${selectedContact.id}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      <div className="containermessagerie" >
        <div className="mainmessagerie">
          <div className="chat-container" style={{  paddingTop: "20px", paddingBottom: "20px" }}>
            <div className="contacts-sidebar" style={{display:"flex"}}>
