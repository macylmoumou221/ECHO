...
const AjoutPub = ({ 
  ...
  const [currentComments, setCurrentComments] = useState(() => {
    ...
    arr.sort((a, b) => new Date(b.createdAt || b.timestamp || 0) - new Date(a.createdAt || a.timestamp || 0));
    return arr;
  })
  ...
  const handleLike = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/api/posts/${postId}/upvote`,
        {},
        {
  ...
  const handleDislike = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/api/posts/${postId}/downvote`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
        }
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      ...
      // optimistic insert: create a temporary comment so the user sees their comment immediately
      ...
      const tempComment = {
        ...
        author: {
          ...
          name: currentUser ? `${currentUser.firstName || ''} ${currentUser.lastName || ''}`.trim() : 'Vous',
      ...
      setCurrentComments(prev => [tempComment, ...prev]);
      // clear input locally while request runs
      ...
      // reconcile: fetch authoritative comments for this post and replace local list
      try {
        const resp = await axios.get(`${BASE_URL}/api/posts/${postId}`, { headers: { Authorization: `Bearer ${token}` } });
        const serverComments = Array.isArray(resp.data.comments) ? resp.data.comments : (resp.data.post?.comments || []);
        ...
        const mapped = serverComments.map(c => {
          ...
          return {
            ...
            hasUpvoted: Array.isArray(c.upvotes) ? c.upvotes.includes(currentUser?.id) : false,
            hasDownvoted: Array.isArray(c.downvotes) ? c.downvotes.includes(currentUser?.id) : false,
            replies: repliesArr.map(r => {
              ...
              return {
                ...
                author: {
                  id: ru._id || ru.id,
                  name: ru.firstName ? `${ru.firstName} ${ru.lastName}`.trim() : (ru.name || ""),
                  username: ru.username || ru.handle || "",
                  avatar: ru.profilePicture || ru.avatar || ""
                },
                ...
                hasUpvoted: Array.isArray(r.upvotes) ? r.upvotes.includes(currentUser?.id) : false,
                hasDownvoted: Array.isArray(r.downvotes) ? r.downvotes.includes(currentUser?.id) : false,
                voteCount: (Array.isArray(r.upvotes) ? r.upvotes.length : 0) - (Array.isArray(r.downvotes) ? r.downvotes.length : 0)
              }
            })
          }
        });

        // ensure most recent comments are first
        mapped.sort((a, b) => new Date(b.createdAt || b.timestamp || 0) - new Date(a.createdAt || a.timestamp || 0));
        setCurrentComments(mapped);
      } catch (err2) {
        console.error('Error reconciling comments after add:', err2);
        // leave optimistic comment in place if reconciliation fails
      }
    } catch (err) {
      console.error("Error adding comment:", err)
      // remove optimistic comment on error
      setCurrentComments(prev => prev.filter(c => !String(c.id).startsWith('temp-')));
  ...
  const handleCommentVote = async (commentId, voteType, isReply = false, parentId = null) => {
    try {
      ...
      const endpoint = isReply 
        ? `${BASE_URL}/api/posts/${postId}/comments/${parentId}/replies/${commentId}/${vote}`
        : `${BASE_URL}/api/posts/${postId}/comments/${commentId}/${vote}`;
  ...
  const handleSaveClick = async (e) => {
    ...
    try {
      ...
      if (isSaved) {
        ...
        await axios.post(`${BASE_URL}/api/users/saved/posts/${postId}`, {}, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
