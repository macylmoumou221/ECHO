...
export default function LostFoundPage() {
  ...
  useEffect(() => {
    const fetchLostFoundItems = async () => {
      try {
        ...
        const response = await axios.get(`${BASE_URL}/api/lostfound`, {
          headers: {
  ...
  const fetchMyItems = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(`${BASE_URL}/api/lostfound/my-items`, {
  ...
  // Helper to get current logged-in user's id from localStorage (supports different shapes)
    ...
    <div className="relative p-4 sm:p-6 md:p-8 pt-20 select-none">
      ...
      >
        {(() => {
          ...
          const ownerId = selectedItem?.reporter?._id || selectedItem?.author?.id || null
          ...
          return (
            <button
              ...
              onClick={() => {
                if (isOwner) {
                  // If the user is the owner, just close the detail modal
                  setIsDetailOpen(false)
                } else {
