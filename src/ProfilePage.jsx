...
import { FaCheck, FaInstagram, FaFacebook, FaGithub, FaLinkedin, FaTiktok, FaTwitter, FaEdit } from "react-icons/fa"
...
export default function ProfilePage({ user }) {
  ...
  const [showGroupPromotionDialog, setShowGroupPromotionDialog] = useState(false)
  const [selectedPromotion, setSelectedPromotion] = useState("")
  const [selectedGroup, setSelectedGroup] = useState("")
  const groupPromotionDialogRef = useRef(null)
  ...
  // Keep local state in sync when parent `user` prop updates (so UI reflects changes instantly)
  useEffect(() => {
    setBio(user?.bio || "")
    ...
    setSocialMedias(user?.socialLinks || [])
  }, [user])
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (groupPromotionDialogRef.current && !groupPromotionDialogRef.current.contains(event.target)) {
        setShowGroupPromotionDialog(false)
        setSelectedPromotion("")
        setSelectedGroup("")
      }
    }

    if (showGroupPromotionDialog) {
  ...
  const saveBio = async () => {
    if (bioInputRef.current) {
      ...
      try {
        // Use refetch which already handles token header
        const res = await refetch('/api/users/profile', 'PUT', { bio: newBio })
        if (res) {
          setHasChanges(false)
          setEditingBio(false)
          // Refresh and broadcast so other components update immediately
          await refreshAndBroadcastUser()
        }
      } catch (err) {
    ...
    if (file) {
      ...
      try {
        ...
        if (response.data?.profilePicture) {
          ...
          // Refresh canonical user and broadcast so UI updates across the app
          await refreshAndBroadcastUser()
        }
      } catch (err) {
  ...
  const handleUrlPhotoChange = async () => {
    if (profilePictureUrl) {
      ...
      try {
        ...
        // Refresh canonical user and broadcast to app so UI updates immediately
        await refreshAndBroadcastUser()
      } catch (err) {
  ...
  // Refresh authoritative user from backend and broadcast to app
  const refreshAndBroadcastUser = async () => {
    try {
      const me = await refetch('/api/auth/me', 'GET')
      if (me) {
        try {
          localStorage.setItem('user', JSON.stringify(me))
        } catch (e) {}
        window.dispatchEvent(new CustomEvent('userUpdated', { detail: me }))
      }
    } catch (err) {
  ...
  const renderRoleSpecificInfo = () => {
    if (user?.role === "teacher") {
      ...
      if (!user?.promotion || !user?.group) {
        return (
          <motion.button
            onClick={() => setShowGroupPromotionDialog(true)}
            className="bg-gradient-to-r from-[#3DDC97] to-[#32c285] text-white px-4 py-2 rounded-full text-sm hover:shadow-lg transition-all shadow-sm w-full"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            Ajouter groupe/promotion
          </motion.button>
        )
      }
        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm w-full">
          ...
          <p className="text-lg font-medium text-center">
            <span className="font-semibold">Groupe:</span>{" "}
              <span className="text-[#3DDC97] font-medium">{user?.group ? (user.group.toString().startsWith('G') ? user.group : `G${user.group}`) : ''}</span>
          </p>
        </div>
      )
    }
            ...
            {post.mediaType != "none" && (
              <div className="relative h-48 w-full overflow-hidden">
                {post.mediaType === "image" ? (
                  <img src={post.media || "/placeholder.svg"} alt={post.title} className="w-full h-full object-cover" />
                ) : (
        ...
        <div className="p-6">
          <div className="flex flex-col md:flex-row items-start gap-6">
            <div className="flex flex-col items-center">
              ...
              >
                <img
                  src={getImageUrl(profilePhoto) || "/placeholder.svg"}
        ...
        <div className="fixed inset-0 z-50">
          ...
          <div className="relative h-full w-full flex items-center justify-center">
            <motion.div
              ...
              transition={{ duration: 0.2 }}
              className="bg-gradient-to-br from-white to-gray-50 rounded-lg shadow-xl p-6 w-96 max-w-[90%] z-10 border-2 border-[#3DDC97]"
              ref={groupPromotionDialogRef}
            >
              <h3 className="text-xl font-semibold mb-4 text-[#2A2A3B] text-center">
                Ajouter votre groupe et promotion
              </h3>

              <div className="mb-4">
                <label className="block text-sm font-medium text-[#2A2A3B] mb-2">Promotion</label>
                <select
                  value={selectedPromotion}
                  onChange={(e) => setSelectedPromotion(e.target.value)}
                  className="w-full p-3 border-2 border-gray-300 rounded-lg text-sm focus:border-[#3DDC97] focus:outline-none focus:ring-2 focus:ring-[#3DDC97]/20"
                >
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-[#2A2A3B] mb-2">Groupe</label>
                <select
                  value={selectedGroup}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-[#2A2A3B] text-white rounded-lg text-sm font-medium hover:bg-[#1f1f2e] transition-colors"
                >
                  Annuler
                </button>
                <button
                  onClick={async () => {
                    if (!selectedPromotion || !selectedGroup) return
                    try {
                      // Send group and promotion to backend
                      const body = { group: selectedGroup, promotion: selectedPromotion }
                      const res = await refetch(`/api/users/profile`, "PUT", body)
                      if (res) {
                        // Refresh canonical user and broadcast
                        await refreshAndBroadcastUser()
                        setShowGroupPromotionDialog(false)
                        setSelectedPromotion("")
                        setSelectedGroup("")
                      ...
                      console.error("Failed to save group/promotion:", err)
                      // keep dialog open to allow retry
                    }
                  }}
                  disabled={!selectedPromotion || !selectedGroup}
                  className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                    selectedPromotion && selectedGroup
                      ? "bg-[#3DDC97] text-white hover:shadow-lg hover:bg-[#32c285]"
